    ...
    try {
        ...
        await saveToUser.save()
            .then(async(admin) => {
                ...
                admin.languageType = await mailLanguagesModel.findOne({languageId:admin.language},{languageType:1});
                let backendApiUrl = config.logoUrl;
                let mailContent = await findMailLanguage(admin.language,"signUp");

                ejs.renderFile(process.cwd()+"/"+config.mailUrl + "email/admin/signUp.ejs", { admin: admin, URL: config.adminUrl,clientMail:config.clientMail,backendApiUrl,mailContent}).then(content => {
