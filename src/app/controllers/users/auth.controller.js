    ...
    try {
        ...
        await saveToUser.save()
            .then(async(user) => {
                ...
                let businessAdmin = await AdminModel.findOne({_id:user.businessId},{language:1,applicationStatus:1});
                
                user.languageType = await mailLanguagesModel.findOne({languageId:businessAdmin.language},{languageType:1});
                user.applicationStatus = businessAdmin.applicationStatus;
                

                let backendApiUrl = config.logoUrl;
                let clientMail = config.clientMail;

                // let mailContent = await findMailLanguage(businessAdmin.language,"signUp");
